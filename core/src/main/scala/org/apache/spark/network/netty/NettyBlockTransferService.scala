...
class NettyBlockTransferService(conf: SparkConf, securityManager: SecurityManager, numCores: Int)
  ...
  override def prepareBlocks(
    ...
    if (prepareBlockIds != null) {
      for (i <- 0 until  prepareBlockIds.size) {
        logInfo(s"send prepare block $i: " + prepareBlockIds(i))
      }
    }

    if (releaseBlockIds != null) {
      for (i <- 0 until  releaseBlockIds.size) {
        logInfo(s"send release block $i: " + releaseBlockIds(i))
      }
    }
    ...
    try{
      ...
      if (maxRetries > 0) {
        new RetryingBlockPreparer(transportConf, blockPrepareStarter, prepareBlockIds, releaseBlockIds, listener).start()
      } else {
