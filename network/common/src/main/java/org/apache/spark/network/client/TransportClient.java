...
public class TransportClient implements Closeable {
  ...
  public void sendRpc(byte[] message, final RpcResponseCallback callback) {
    ...
    logger.debug("BM@Sending RPC {} to {}", new String(message), serverAddr);

    final long requestId = Math.abs(UUID.randomUUID().getLeastSignificantBits());
    handler.addRpcRequest(requestId, callback);

    logger.debug("BM@Sending RPC request add to handler success");
      ...
      new ChannelFutureListener() {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
          if (future.isSuccess()) {
            ...
            logger.debug("Sending request {} to {} took {} ms", requestId, serverAddr, timeTaken);
          } else {
